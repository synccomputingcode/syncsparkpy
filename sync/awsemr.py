...
def get_project_job_flow(job_flow: dict, project_id: str) -> Response[dict]:
    ...
    if _project:
        ...
        s3_url = _project.get("cluster_log_dest")
        if s3_url:
...
def get_project_cluster_report(  # noqa: C901
    ...
    project = project_response.result
    if project:
        project_url = project.get("cluster_log_dest")
        if project_url:
...
def run_job_flow(job_flow: dict, project_id: str = None, region_name: str = None) -> Response[str]:
    ...
    if project_id:
        ...
        if project_response.error:
            return project_response
        project = project_response.result
        if project.get("cluster_log_dest"):
            match = re.match(
                RUN_DIR_PATTERN_TEMPLATE.format(
                    project_prefix=project["cluster_log_dest"], project_id=project_id, run_id=run_id
                ),
            ...
            if match:
                ...
                run_dir = f"{project['cluster_log_dest']}/{project['id']}/{datetime.datetime.utcnow().strftime(TIME_FORMAT)}/{run_id}"
...
def _get_existing_run_dir(
    ...
    project = project_response.result
    if project:
        s3_url = project.get("cluster_log_dest")
        if s3_url:
