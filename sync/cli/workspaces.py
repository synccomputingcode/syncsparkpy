...
def create_workspace_config(
    ...
    databricks_host = click.prompt(
        "Databricks host (prefix with https://)", default=get_databricks_config().host if get_databricks_config() else None
    )
    ...
    sync_api_key_id = click.prompt("Sync API key ID", default=get_api_key().id if get_api_key() else None)
